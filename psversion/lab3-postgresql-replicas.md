...
kubectl exec -it postgresql-client -n postgresql -- psql -h postgresql-replica-0.postgresql-headless -U postgres -d testdb -c "INSERT INTO sample_data (name) VALUES ('This should fail');"
if (-not $?) { Write-Host "Expected: Write operation failed on replica" }
```
...
kubectl exec -it postgresql-replica-0 -n postgresql -- psql -U postgres -c "SELECT pg_promote();"
if (-not $?) { Write-Host "Promotion command may vary by PostgreSQL version" }
...
Write-Host "Note: In production, you would need to rebuild the original primary from the promoted replica"
...
kubectl get pods -n postgresql -o wide | Select-String postgresql
```
...
1. **PostgreSQL "root execution not permitted" Error**
   ...
   kubectl describe pod postgresql-primary-0 -n postgresql | Select-String "Security Context" -Context 0,10
...
kubectl get pv | Select-String postgresql
